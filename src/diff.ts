...
export function parseUnifiedDiff(diff: string): DiffLine[] {
  ...
  for (const rawLine of diff.split("\n")) {
    ...
    if (rawLine.startsWith("+")) {
      result.push({ path, side: "RIGHT", line: newLine, text: rawLine.slice(1) });
      newLine += 1;
    } else if (rawLine.startsWith("-")) {
      result.push({ path, side: "LEFT", line: oldLine, text: rawLine.slice(1) });
      oldLine += 1;
    } else if (rawLine.startsWith(" ")) {
      const text = rawLine.slice(1);
      result.push({ path, side: "LEFT", line: oldLine, text });
      result.push({ path, side: "RIGHT", line: newLine, text });
      oldLine += 1;
      newLine += 1;
    }
  }
...
export function assertDiffTarget(
  ...
  const exists = parseUnifiedDiff(diff).some(
    (candidate) =>
      candidate.path === path &&
      candidate.side === side &&
      candidate.line === line
